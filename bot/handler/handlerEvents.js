...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	return async function (event, message) {
		...
		async function onChat() {
			...
			const noprefix = threadData.data?.noprefix;

			if (!noprefix) return;

			const firstWord = body.split(" ")[0].toLowerCase();
			const args = body.slice(firstWord.length).trim().split(/ +/);

			// Case 1: ALL commands enabled
			if (noprefix.enabled === true) {
				const command = GoatBot.commands.get(firstWord);
				if (command && command.onStart) {
					...
					try {
						...
						log.info("NO-PREFIX COMMAND", `${commandName} | ${userData.name} | ${senderID} | ${threadID} | ${args.join(" ")}`);
					} catch (err) {
						log.err("NO-PREFIX COMMAND", `Error in command ${commandName}`, err);
					}
				}
			}
			// Case 2: Specific commands enabled
			else if (noprefix.commands && Array.isArray(noprefix.commands) && noprefix.commands.length > 0) {
				if (noprefix.commands.includes(firstWord)) {
					const command = GoatBot.commands.get(firstWord);
					if (command && command.onStart) {
						const commandName = command.config.name;
