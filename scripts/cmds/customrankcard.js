    ...
    try {
      ...
      if ([...notSupportImage, ...supportImage].includes(key)) {
        ...
        if (value === 'reset') {
          ...
          try {
            const imgbbResponse = await uploadToImgBB(matchUrl[0]);
            value = imgbbResponse;
          } catch (uploadError) {
          ...
          try {
            const imgbbResponse = await uploadToImgBB(attachments[0].url);
            value = imgbbResponse;
          } catch (uploadError) {
            console.error("ImgBB upload error:", uploadError);
            return message.reply(getLang("error", "Failed to upload image"));
          }
        }
        else {
        ...
        try {
          const rankCardStream = await global.client.makeRankCard(
            event.senderID, 
            usersData, 
            threadsData, 
            event.threadID, 
          ...
          rankCardStream.path = "rankcard.png";

          await message.reply({
            body: getLang("success"),
            attachment: rankCardStream
          });
        } catch (rankError) {
          console.error("Rank card generation error:", rankError);
        ...
        await message.reply(getLang("reseted"));
      }
      else {
        message.SyntaxError();
      }
    } catch (err) {
      console.error("CustomRankCard Error:", err);
...
async function uploadToImgBB(imageUrl) {
  try {
    const API_KEY = "6ca2869a7907340bc14f52d3931b4a91"; // You need to add your ImgBB API key here
    const response = await axios.get(`https://api.imgbb.com/1/upload?key=${API_KEY}&image=${encodeURIComponent(imageUrl)}`);
