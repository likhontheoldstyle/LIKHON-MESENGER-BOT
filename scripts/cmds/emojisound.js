...
module.exports = {
  ...
  onStart: async function ({ event, message, args }) {
    if (args[0] && args[0].toLowerCase() === "list") {
      ...
      const emojiList = Object.keys(emojiAudioMap);
      let messageText = "🎵 Emoji Voice List 🎵\n\n";
      
      emojiList.forEach((emoji, index) => {
        const audioCount = emojiAudioMap[emoji].length;
        messageText += `${index + 1}. ${emoji} - ${audioCount} voices\n`;
      });
      
      messageText += `\n📊 Total Emojis: ${emojiList.length}`;
      messageText += `\n\nHuh!`;
